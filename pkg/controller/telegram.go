...
func notifyTelegram(cluster string, env string, imageName string, oldTag string, newTag string) {
	...
	telegramMessage := "<b>VNGITBOT-V2 has changed version tag for deployment.</b>%0A<b>Service</b>: <code>" + service + "</code>%0A<b>Cluster</b>: <code>" + cluster + "-" + env + "</code>%0A<b>Old tag</b>: <code>" + oldTag + "</code>  ==>  <b>New tag</b>: <code>" + newTag + "</code>"
